...
function run_trafgen() {
    ...
    echo "kubectl exec \"$tx_pod_name\" -- timeout \"${DEFAULT_TIMEOUT}s\" $cmd /usr/local/sbin/trafgen --cpp --dev \"$DEFAULT_IF_NAME\" -i \"$trafgen_udp_file\" --no-sock-mem --rate \"${pps}pps\" --bind-cpus 2-19,22-39 -V -H > /dev/null 2>&1 &"
    kubectl exec "$tx_pod_name" -- timeout "${DEFAULT_TIMEOUT}s" $cmd /usr/local/sbin/trafgen --cpp --dev "$DEFAULT_IF_NAME" -i "$trafgen_udp_file" --no-sock-mem --rate "${pps}pps" --bind-cpus 2-19,22-39 -V -H > /dev/null 2>&1 &
    trafgen_pid=$!
}
...
function run_trafgen_inter_pod() {
    ...
    for i in "${!tx_pod_names[@]}"; do
        ...
        echo "Starting on pod $_tx_pod_name with core $default_core and pps "\
        ...
        echo "kubectl exec \"$_tx_pod_name\" -- timeout \"${DEFAULT_TIMEOUT}s\" /usr/local/sbin/trafgen --cpp --dev \"$DEFAULT_IF_NAME\" -i \"$trafgen_udp_file2\" --no-sock-mem --rate \"${pps}pps\" --bind-cpus 2-19,22-39 -H > /dev/null 2>&1 &"

        kubectl exec "$_tx_pod_name" -- timeout "${DEFAULT_TIMEOUT}s" /usr/local/sbin/trafgen --cpp --dev "$DEFAULT_IF_NAME" -i "$trafgen_udp_file2" --no-sock-mem --rate "${pps}pps" --bind-cpus 2-19,22-39 -H > /dev/null 2>&1 &
...
function run_monitor_all() {
  for i in "${!rx_pod_names[@]}"; do
     ...
     local _default_core="${default_cores[$i]}"
     echo "Starting monitor on pod $_rx_pod_name with core $default_core"
     kubectl exec "$_rx_pod_name" -- timeout "${DEFAULT_MONITOR_TIMEOUT}s" \
     /tmp/monitor_pps.sh -i net1 > "/tmp/monitor_${_rx_pod_name}.log" 2>&1 &
    done
...
function collect_pps_rate_all() {
  ...
  for pod_id in "${server_pods[@]}"; do
      ...
      kubectl exec "$pod_id" -- timeout "${DEFAULT_MONITOR_TIMEOUT}s" \
      /tmp/monitor_pps.sh -i net1 -d tuple> "$output_file" &
      ((pod_ith++))
  done
}
...
function get_geneve_interface_stats() {
    ssh root@"$tx_node_addr" cat /proc/net/dev | grep 'genev_sys' > "$output_file" &
}

# This a pod interface server0--63ab25
function get_pod_interface_stats() {
    ssh root@"$tx_node_addr" cat /proc/net/dev | grep $target_pod_name > "$output_file" &
}

# this a pod interface stats for Andrea
function get_antrea_interface_stats() {
    ssh root@"$tx_node_addr" cat /proc/net/dev | grep antrea-gw0 > "$output_file" &
}
...
function collect_queue_rates() {
    ...
    local tx_soft_net_log="${output_dir}/tx-softnet-stat_${pps}_runtime_${DEFAULT_TIMEOUT}_cores_${num_cores}_pairs_${num_pairs}_size_${packet_size}_ts_${timestamp}.log"
    local rx_soft_net_log="${output_dir}/rx-softnet-stat_${pps}_runtime_${DEFAULT_TIMEOUT}_cores_${num_cores}_pairs_${num_pairs}_size_${packet_size}_ts_${timestamp}.log"

    echo "Collecting queue rates and CPU utilization from workers at $timestamp..."
#    ssh root@"$tx_node_addr" timeout "${DEFAULT_MONITOR_TIMEOUT}s" \
#    /bin/bash /tmp/monitor_queue_rate.sh -t queue > "$tx_queue_output_file" 2>&1 &
    ...
    echo "Collecting queue rates and CPU utilization from workers at $timestamp..."
#    ssh root@"$rx_node_addr" timeout "${DEFAULT_MONITOR_TIMEOUT}s" \
#    /bin/bash /tmp/monitor_queue_rate.sh -t queue > "$rx_queue_output_file" 2>&1 &
    ...
    ssh root@"$tx_node_addr" timeout "${DEFAULT_MONITOR_TIMEOUT}s" \
    python3 /tmp/monitor_softnet_stat.py --concise -c > "$tx_soft_net_log" 2>&1 &
    ssh root@"$rx_node_addr" timeout "${DEFAULT_MONITOR_TIMEOUT}s" \
    python3 /tmp/monitor_softnet_stat.py --concise -c > "$rx_soft_net_log" 2>&1 &
}
...
function collect_tx_rx_int() {
    ...
    ssh root@"$tx_node_addr" timeout "${DEFAULT_MONITOR_TIMEOUT}s" /bin/bash /tmp/monitor_txrx_int.sh > "$tx_output_file" &
    ...
    ssh root@"$rx_node_addr" timeout "${DEFAULT_MONITOR_TIMEOUT}s" /bin/bash /tmp/monitor_txrx_int.sh > "$rx_output_file" &
}

# this a pod interface stats for uplink
function get_interface_stats() {
    ssh root@"$tx_node_addr" cat /proc/net/dev | grep "$uplink_interface"
}
...
function get_and_print_interface_stats() {
    local if_name
    if_name=$1
    interface_stats=$(ssh root@"$tx_node_addr" cat /proc/net/dev | grep "$if_name")
