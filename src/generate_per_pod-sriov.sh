...
DEST_IPS=($(kubectl get pods -l role=client -oyaml |grep 192.168.0.|awk '{ print $1 }'|tr -d \"))
echo "------>DEST_IPS: ${DEST_IPS}"
SERVER_IPS=($(kubectl get pods -l role=server -oyaml |grep 192.168.0.|awk '{ print $1 }'|tr -d \"))
...
execute_in_parallel() {
    ...
    for pod in "${pods[@]}"; do
          ...
          echo "----->execute-in-parallel $dst_addr"
          SRC_PORT=$((1024 + pod_id))
          DST_PORT=$((1024 + pod_id))
        (
	    echo "RDO RDO - ${PD_SIZE} ${SRC_PORT} ${DST_PORT}"
            kubectl cp pkt_generate_template-sriov.sh "$pod":/tmp/pkt_generate_template.sh
            ...
            if [ "$pod_id" -eq 0 ]; then
                ...
                echo "----->execute-in-parall-el $dest_ip_loopback"
...
function generate_traffic_profile() {
  local payload_sizes
    payload_sizes=(64 128 256 512 1024 1518)
    local frame_size
