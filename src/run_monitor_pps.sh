...
function run_trafgen() {
    ...
    echo "kubectl exec \"$tx_pod_name\" -- timeout \"${DEFAULT_TIMEOUT}s\" $cmd /usr/local/sbin/trafgen --cpp --dev \"$DEFAULT_IF_NAME\" -i \"$trafgen_udp_file\" --no-sock-mem --rate \"${pps}pps\" --bind-cpus \"$default_core\" -V -H > /dev/null 2>&1 &"
...
function run_trafgen_inter_pod() {
    ...
    for i in "${!tx_pod_names[@]}"; do
        ...
        kk="kubectl exec \"$_tx_pod_name\" -- timeout \"${DEFAULT_TIMEOUT}s\" \\
        /usr/local/sbin/trafgen --cpp --dev \"$DEFAULT_IF_NAME\" -i \\
        \"$trafgen_udp_file2\" --no-sock-mem --rate \"${pps}pps\" --bind-cpus \"$default_core\" -H > /dev/null 2>&1 &"
	echo $kk

        kubectl exec "$_tx_pod_name" -- timeout "${DEFAULT_TIMEOUT}s" /usr/local/sbin/trafgen --cpp --dev "$DEFAULT_IF_NAME" -i "$trafgen_udp_file2" --no-sock-mem --rate "${pps}pps" --bind-cpus "$default_core" -H > /dev/null 2>&1 &
