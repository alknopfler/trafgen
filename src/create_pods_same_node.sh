...
do
    ...
    sed "s/{{client-name}}/$client_name/g; s/{{node-name}}/$client_node/g; s/{{server-name}}/$server_name/g; s/{{cpu-limit}}/$cpu_limit/g; s/{{memory-limit}}/$memory_limit/g; s/{{cpu-request}}/$cpu_request/g; s/{{memory-request}}/$memory_request/g" pod-client-template-samenode.yaml > "pods/pod-$client_name.yaml"
